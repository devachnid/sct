...
* [~] **Same-hardware `sct serve` vs CSIRO Ontoserver comparison.** A reference-grade **Ontoserver** and **`sct serve`** now run on the *same box, same hardware* (a shared instance administered by the maintainer), reachable as two FHIR bases behind one host (`sct serve` at `<BASEURL>/sct/fhir`). This is the strongest external comparator available: Ontoserver is the reference commercial SNOMED terminology server that `sct serve` is a lightweight alternative to, and co-locating them removes the network / RTT / hardware confounders that make a laptop-vs-remote comparison meaningless - any latency delta is the software, not the environment. The Ontoserver side is loaded with **SNOMED CT July 2026** plus an **ICD-10 crossmap**, so `$lookup` / `$validate-code` / `$subsumes` / `$expand` (incl. ECL) and `ConceptMap/$translate` (SNOMED CT → ICD-10) are all directly comparable. Wiring is already in place: run `benchmarks/conformance.sh --server <ontoserver-base>` first (never benchmark a server that fails the profile), then `benchmarks/bench.sh --sct-fhir <BASEURL>/sct/fhir --vs <ontoserver-base>` for a FHIR-to-FHIR, like-for-like run; publish the results alongside the fully-owned Snowstorm Lite numbers. **The definitive/public run executes the benchmark client *on the box itself*, over loopback to both servers** - either their published container ports, or the public URLs pinned to `127.0.0.1` (temporary hosts override) so both still route through the same local reverse proxy. Running from a laptop leaves ~100 ms of wide-area latency sitting near-equally on *both* sides, which compresses every ratio toward 1.0× and hides the real server-compute delta (a co-located client removes it); a first laptop run already showed `sct` marginally ahead on the low-variance single-query operations even *with* that network tax on both sides. **Constraints:** it is a *live, shared* instance, so single-request latency and conformance runs are fine but the concurrent load-testing harness (above) must **not** be pointed at it without coordination - it would contend with real users; use a disposable/owned instance for heavy load runs. Benchmark scripts must take the base URLs as arguments and **never hardcode the host or any credentials** into committed files.
