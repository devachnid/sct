...
- [x] `R1` **Progress + ETA for the long-running builds.** A shared `progress` module (`src/progress.rs`) provides three consistent widgets - a spinner for opaque/indeterminate phases, a byte-oriented bar for streaming a file of known size (ETA derived from the byte rate, wired through `ProgressBar::wrap_read` so it advances with no per-loop bookkeeping), and a count-oriented bar for loops over a known number of items. Applied across the build commands: `sqlite`, `parquet`, `markdown`, and `fst build` show a byte bar with a live ETA as they stream the NDJSON; `tct` and `embed` show a count bar with a per-item ETA over their known totals; `ndjson` shows a per-file byte bar for every RF2 section it reads (concepts, descriptions, relationships, refsets, maps, associations - via the shared `tsv_reader`), then a count bar while it builds the concept records, then a count bar on the final write, each under the loader's existing line-by-line breadcrumb. All widgets draw to stderr and auto-hide when stderr is not an interactive terminal (piped, redirected, or CI / `TERM` unset), so machine-readable stdout and CI logs stay clean. Verified on a real PTY: e.g. `sct sqlite` renders `5.41 MiB/162.16 MiB (5.9 MiB/s, ETA 26s)` and `sct tct` renders `106,664/250,000 (442,805/s)`.
- [x] `R2` **`--input` renamed to `--ndjson`, with a file-path completion hint.** The five NDJSON-consuming commands (`sqlite`, `parquet`, `markdown`, `embed`, `fst build`) now take `--ndjson <NDJSON>` and carry `value_hint = FilePath`, so the generated shell completions (`sct completions`) offer file paths for the argument. `--input` and `-i` are kept as a visible alias, so existing scripts and muscle memory keep working. `sct map --input` is deliberately left alone - it reads a stream of codes, not an NDJSON artefact, so the `--ndjson` name would mislead. On the a/b/c question: (a) a self-documenting per-artefact flag name is the feasible, universal win and is now shipped; (c) it is paired with generic file-path completion via `value_hint`; (b) extension-filtered completion (`*.ndjson` only) is not portable across clap's supported shells, so we did not attempt it. Docs migrated to `--ndjson` throughout (with the alias noted).
...
- [~] `R4` **Concurrent load-testing harness for `sct serve`.** **Shipped:** `benchmarks/load.sh` drives `sct serve` under sustained concurrency via `oha` (keep-alive, JSON), ramping the client count (1, 2, 4, 8, 16, 32, 64, 128) across `$lookup` / `$validate-code` / `$subsumes` / `$expand` and reporting **throughput (req/s), tail latency (p50/p95/p99/p99.9), and error rate** per level plus the saturation peak; `--write-report` and optional `--stat-container` (server memory under load) included. See `benchmarks/README.md`. A single-server test with no comparator, so results are publishable. **Scaling bottleneck found and fixed:** the co-located run showed throughput peaking at ~4 clients then *declining* - the predicted signature of the connection-per-request design (`open_db_readonly` per request, so every request paid a file open + pragma + SQL recompile, and the `prepare_cached` calls in `ops.rs` never actually cached). `sct serve` now borrows from a **bounded pool of warm read-only connections** (`serve/pool.rs`, `--pool-size`, default 2x logical cores clamped 4..64) with memory-mapped reads (`PRAGMA mmap_size` in `open_db_readonly`, which also speeds every CLI read path). A controlled A/B on the same box and the same 837k-concept Monolith DB: `$lookup` peak **2,967 -> 28,777 req/s (~10x)** with the curve inverted from decline to climb (peak moved from 4 to 64 clients), and p50 at 128 clients **61 ms -> 4 ms**; ECL `$expand` (CPU-bound, so connection overhead is a smaller share) **~2x**, its peak moving 4 -> 16 clients. **Remaining:** run it on a real box with the client separate from the server, and publish the latency-vs-concurrency / throughput-vs-concurrency curves.
...
- [ ] `R22` **Doctests on library public items** - now that `src/lib.rs` exposes `build_records`, `Rf2Dataset::load`, the rf2 parsers, etc. as genuine library surface, `///` examples on these items double as living documentation and get tested by `cargo test` for free.
...
**One search core, three front-ends - shipped.** All three surfaces are built over one shared core, [`Index::search_typeahead`](../src/index/query.rs) (mmap'd [FST index](commands/fst.md), sub-millisecond prefix + multi-word intersection + optional typo-tolerant fuzzy). See [`docs/commands/sayt.md`](../docs/commands/sayt.md).

- [x] `R28` **1. Interactive TUI - the demo.** ✅ `sct sayt` - a full-screen mode (ratatui, `--features tui`) that repaints on every keystroke with a live per-keystroke latency readout; ↑↓ select, Enter emits `SCTID⇥term` to stdout for piping, Esc quits.
- [x] `R29` **2. stdio line protocol - embed in a native app.** ✅ `sct sayt --stdio` (works in any build) - one query per line on stdin, one JSON line (`{query, hits:[{id,display,score,tag}]}`) per query on stdout, flushed immediately. `id` is a string (SCTIDs exceed 2^53).
- [x] `R30` **3. HTTP autocomplete endpoint - the web-native surface.** ✅ `GET /autocomplete?q=…&count=…` on [`sct serve`](commands/serve.md) (start it with `--fst`, or a `snomed.fst` beside the DB), same JSON shape; `501` when no FST index is loaded. The drop-in "autocomplete for 800k SNOMED concepts, sub-ms, offline, no Elasticsearch".

**Remaining refinements** (the surfaces work; these sharpen them): frequency-weighted / clinical-relevance ranking instead of the current length-proximity heuristic; a word-prefix pass so mid-typing multi-word queries (`diabetes typ…`) match before the last word completes; and a backend toggle (FST / FTS5 / semantic) and hierarchy/semantic-tag filters in the TUI. (The `tui` feature is now in the default build, so `sct sayt`/`sct tui` ship in the released binaries and `cargo install sct-rs`; the Docker server image opts out via `--no-default-features`.)
