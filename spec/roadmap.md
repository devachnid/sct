...
- [x] **1. Interactive TUI - the demo.** ✅ `sct sayt` - a full-screen mode (ratatui, `--features tui`) that repaints on every keystroke with a live per-keystroke latency readout; ↑↓ select, Enter emits `SCTID⇥term` to stdout for piping, Esc quits.
- [x] **2. stdio line protocol - embed in a native app.** ✅ `sct sayt --stdio` (works in any build) - one query per line on stdin, one JSON line (`{query, hits:[{id,display,score,tag}]}`) per query on stdout, flushed immediately. `id` is a string (SCTIDs exceed 2^53).
- [x] **3. HTTP autocomplete endpoint - the web-native surface.** ✅ `GET /autocomplete?q=…&count=…` on [`sct serve`](commands/serve.md) (start it with `--fst`, or a `snomed.fst` beside the DB), same JSON shape; `501` when no FST index is loaded. The drop-in "autocomplete for 800k SNOMED concepts, sub-ms, offline, no Elasticsearch".
