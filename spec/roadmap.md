...
**Remaining refinements** (the surfaces work; these sharpen them): frequency-weighted / clinical-relevance ranking instead of the current length-proximity heuristic; a word-prefix pass so mid-typing multi-word queries (`diabetes typ…`) match before the last word completes; a backend toggle (FST / FTS5 / semantic) and hierarchy/semantic-tag filters in the TUI; and shipping the interactive TUI in the released binaries (it is currently `tui`-gated, so absent from the default release build - a decision, since it adds ratatui to the shipped artefact).
